...
# vdsl speeds
$output = `rrdtool graph $graphdirectory/wan${time}.png -a PNG --vertical-label "kbit/sec" -s -${time} -w 1024 -h 300 'DEF:up='$rrddirectory/wan0up.rrd:kbit:LAST 'DEF:down='$rrddirectory/wan0down.rrd:kbit:LAST '${linetype}:down#${col01}:vdsl downstream sync rate' '${linetype}:up#${col02}:upstream' -W "${datestamp}" -t "wan"`;

# vdsl nat entries
$output = `rrdtool graph $graphdirectory/wan-nat${time}.png -a PNG --vertical-label "entries" -s -${time} -w 1024 -h 300 'DEF:nats='$rrddirectory/wan0nat.rrd:nats:LAST '${linetype}:nats#${col01}:cpe router' -W "${datestamp}" -t "nat table entries"`;

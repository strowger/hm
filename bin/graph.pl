...
#$output = `rrdtool graph $graphdirectory/power${filename}.png -a PNG -l 0 -y 50:5 --vertical-label "watts" -s ${starttime} -e ${endtime} -w 1024 -h 300 'DEF:optical='$rrddirectory/ccoptiwatts.rrd:power:LAST 'DEF:clamp='$rrddirectory/ccclampwatts.rrd:power:LAST 'DEF:car='$rrddirectory/ccclampwattscar.rrd:power:LAST  '${linetype}:optical#${col01}:house - pulse-counter reading' '${linetype}:clamp#${col02}:house - clamp meter reading' '${linetype}:car#${col03}:car charger' -W "${datestamp}" -t "electricity consumption"`;

$output = `rrdtool graph $graphdirectory/power${filename}.png -a PNG -l 0 -y 50:5 --vertical-label "watts" -s ${starttime} -e ${endtime} -w 1024 -h 300 'DEF:optical='$rrddirectory/ccoptiwatts.rrd:power:LAST 'DEF:car='$rrddirectory/ccclampwattscar.rrd:power:LAST 'DEF:wash='$rrddirectory/cciamwasher.rrd:power:LAST 'DEF:dry='$rrddirectory/cciamdryer.rrd:power:LAST 'DEF:fridge='$rrddirectory/cciamfridge.rrd:power:LAST '${linetype}:optical#${col01}:whole house' 'AREA:car#${col02}:car charger' 'AREA:fridge#${col03}:fridge':STACK 'AREA:wash#${col04}:washing machine':STACK 'AREA:dry#${col05}:tumble dryer':STACK -W "${datestamp}" -t "electricity consumption"`;
